...
def file_inout():
    try:
        ...
        file_inout_n = int(input(":"))
        if file_inout_n == 1:
            file_addreess_t = str(input("경로를 입력하십시요"))
            with open(file_addreess_t + os_delimeter + json_file_name, 'w', encoding="utf-8") as make_file:
                readable_result = json.dumps(open_json_file(), indent=4, ensure_ascii=False)
                make_file.write(readable_result)
                print('등록된 경로로가 저장되었습니다.')
                final_address = file_addreess_t + os_delimeter + json_file_name,
        ...
        if file_inout_n == 2:
            with open(os.getcwd() + os_delimeter + json_file_name, 'w', encoding="utf-8") as make_file:
                ...
                make_file.write(readable_result)
                print('기본경로로 저장되었습니다.')
                final_address = os.getcwd() + os_delimeter + json_file_name
                return final_address
