        ...
        try:
            while True:
                ...
                if input_sentence =="전체학생조회" or input_sentence =="2":
                    for i in range(len(open_json_file())):
                        ...
                        for x in range(len(open_json_file()[i]["수강정보"])-1):
                            print((("\t"*14)+"강좌"+str(x+1))+"\n\n"+
                                  (("\t"*13)+"강의코드" + str(x+1) +": "+ "%s" % open_json_file()[i]["수강정보"]["강좌" + str(x)]["강의코드"])+"\n"+
                                  (("\t"*13)+"강의명" + str(x+1) + ": " + "%s" % open_json_file()[i]["수강정보"]["강좌" + str(x)]["강의명"])+"\n"+
                                  (("\t"*13)+"강사" + str(x+1) + ": " + "%s" % open_json_file()[i]["수강정보"]["강좌" + str(x)]["강사"])+"\n"+
                                  (("\t"*13)+"개강일" + str(x+1) + ": " + "%s" % open_json_file()[i]["수강정보"]["강좌" + str(x)]["개강일"])+"\n"+
                                  (("\t"*13)+"종료일" + str(x+1) + ": " + "%s" % open_json_file()[i]["수강정보"]["강좌" + str(x)]["종료일"])+"\n")
                            continue
                ...
                if input_sentence == "인적사항조회" or input_sentence == "1":
                    while True:
                        print("\n조회할 ID, 이름, 나이, 주소 중 한가지를 입력하세요 (뒤로가기:0).")
                        print("ex) 김기정\n")
                        personal_sentence = str(input(">>"))
                        if personal_sentence == "0":
                            break
                        else:
                            for i in range(len(open_json_file())):
                                if personal_sentence == open_json_file()[i]["학생ID"] or personal_sentence == open_json_file()[i]["학생정보"]["이름"] or personal_sentence == open_json_file()[i]["학생정보"]["나이"] or personal_sentence == open_json_file()[i]["학생정보"]["주소"] :
                                    print("\n")
                                    print((("\t" * 13) + "학생ID: %s\n" % open_json_file()[i]["학생ID"]) + "\n" +
                                          (("\t" * 12) + "이름: %s" % open_json_file()[i]["학생정보"]["이름"]) + "\n" +
                                          (("\t" * 12) + "나이: %s" % open_json_file()[i]["학생정보"]["나이"]) + "\n" +
                                    ...
                                    for x in range(len(open_json_file()[i]["수강정보"])-1):
                                        ...
                                        continue
                            if not personal_sentence == open_json_file()[i]["학생ID"] or not personal_sentence == open_json_file()[i]["학생정보"]["이름"] or not personal_sentence ==open_json_file()[i]["학생정보"]["나이"] or not personal_sentence ==open_json_file()[i]["학생정보"]["주소"]:
                                print("입력을 잘못 하셨거나, 관련된 데이터가 없습니다.")

                if input_sentence == "수강정보조회" or input_sentence == "3":
                    while True:
                        print("\n조회할 과거 수강 횟수, 강의, 강의코드, 강의명, 강사 중 한가지를 입력하세요.(뒤로가기:0)")
                        print("ex) IT교육\n")
                        institute_sentence = str(input(">>"))
                        if institute_sentence == "0":
                            break
                        else:
                            for i in range(len(open_json_file())):
                                for x in range(len(open_json_file()[i]["수강정보"]) - 1):
                                    if institute_sentence == open_json_file()[i]["수강정보"]["과거수강횟수"] or institute_sentence == open_json_file()[i]["수강정보"]["강좌" + str(x)]["강의코드"] or institute_sentence == open_json_file()[i]["수강정보"]["강좌" + str(x)]["강의명"] or institute_sentence == open_json_file()[i]["수강정보"]["강좌" + str(x)]["강사"]:
                                        print("\n")
                                        print((("\t" * 13) + "학생ID: %s\n" % open_json_file()[i]["학생ID"]) + "\n" +
                                              (("\t" * 12) + "이름: %s" % open_json_file()[i]["학생정보"]["이름"]) + "\n" +
                                              (("\t" * 12) + "나이: %s" % open_json_file()[i]["학생정보"]["나이"]) + "\n" +
                            ...
                            if fix_sentence == "2" or fix_sentence == "수강 정보" or fix_sentence == "수강정보":
                                while True:
                                    ...
                                    if ins_fix_num == 2:
                                        print("-" * 140)
                                        print("강의코드를 선택하셨습니다")
                                        print("현재 강의코드는", "<",(json_big_data[i]["수강정보"]["강좌" + str(x)]["강의코드"]),">", "입니다")
                                        print("-" * 140)
                                    ...
                                    if ins_fix_num == 3:
                                        print("-" * 140)
                                        print("강의명를 선택하셨습니다")
                                        print("현재 강의명은", "<",(json_big_data[i]["수강정보"]["강좌" + str(x)]["강의명"]),">", "입니다")
                                        print("-" * 140)
                                    ...
                                    if ins_fix_num == 4:
                                        print("-" * 140)
                                        print("강사를 선택하셨습니다")
                                        print("현재 강사는", "<",(json_big_data[i]["수강정보"]["강좌" + str(x)]["강사"]),">", "입니다")
                                        print("-" * 140)
                                    ...
                                    if ins_fix_num == 5:
                                        print("-" * 140)
                                        print("개강일를 선택하셨습니다")
                                        print("현재 개강일은","<",(json_big_data[i]["수강정보"]["강좌" + str(x)]["개강일"]),">", "입니다")
                                        print("-" * 140)
                                    ...
                                    if ins_fix_num == 6:
                                        print("-" * 140)
                                        print("종료일를 선택하셨습니다")
                                        print("현재 종료일은","<", (json_big_data[i]["수강정보"]["강좌" + str(x)]["종료일"]),">","입니다")
                                        print("-" * 140)
                ...
                for x in range(len(open_json_file()[i]["수강정보"]) - 1):
                    if input_value == (json_big_data[i]["학생ID"]):
                        while True:
                            ...
                            if del_number == 2:
                                print("-" * 140)
                                print(("해당된 ID의 수강정보 입니다").center(120))
                                print("-" * 140)
                                for i in range(len(json_big_data)):
                                    for x in range(len(open_json_file()[i]["수강정보"]) - 1):
                                        academy_value = list(json_big_data[i]['수강정보'].keys())
                                        print(academy_value[i+1],":",json_big_data[i]['수강정보']['강좌'+str(x)]['강의코드'])
