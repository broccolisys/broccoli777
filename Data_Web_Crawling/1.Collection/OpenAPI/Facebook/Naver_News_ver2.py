...
for i in range(len(json_big_data)):
    ...
    json_entirelist.append(json_split[0])

json_uniquelist = set(json_entirelist)
json_uniquelist.remove('')

json_dict={}
print("[도메인별 기사 건수 분석]")
for i in json_uniquelist:
    global count
    count = 0
    for x in json_entirelist:
        if i == x :
            count += 1
        else:
            continue
...
for i in json_dict_sorted:
    json_dict_total +=json_dict[i]
    print(">>"+i+" : "+ str(json_dict[i]))
print("-"*40)
print("총건수 : "+str(json_dict_total))
